...
#include "asv_simulator/Surface.hh"
...
class asv::Surface::Implementation
{
  ...
  const std::shared_ptr<const sdf::Element> &_sdf)
{
  ...
  while (pointElem)
  {
    ...
    {
      gzerr << "Unable to parse <points><point> as Vector3d." << std::endl;
    }
...
GZ_ADD_PLUGIN(asv::Surface,
              sim::System,
              Surface::ISystemConfigure,
              Surface::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(asv::Surface,
                    "asv::Surface")
